<%@page language="java" import="java.util.*,java.sql.*,java.net.*,bean.orders" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!doctype html>
<html lang="en">
<% 
				...
				<div class="container-fluid">
					...
					<div class="panel panel-headline">
						<div class="panel-body no-padding">
							<form class="navbar-form navbar-left" method="post" action="order_search">
								<div class="input-group">
									<input type="text" value="" class="form-control" placeholder="搜索订单" name="orders_search">
									<span class="input-group-btn"><button type="submit" class="btn btn-primary">Go</button></span>
								</div>
								<img src="assets/img/little_blank.png">
								<a href="orders_add.jsp"><button type="button" class="btn btn-success btn-toastr" data-context="success">添加</button></a>
							</form>
						</div>
					</div>
					<%		
				    ...
				    String oid="",cid="",level="",eid="",mname="",qty="",otime="",a_discount="",final_price="",staname="";
				    while(it.hasNext()){
					       orders rs = it.next();	
					       oid=rs.get_oid();
				    	   cid=rs.get_cid();
						   ...
						   mname=rs.get_mname();
						   qty=rs.get_qty();
						   otime=rs.get_otime().substring(0, 10);
						   a_discount=rs.get_a_discount();
						   final_price=rs.get_final_price();
						   staname=rs.get_staname();
					%>
							...
							<table class="table table-striped">
								...
								<tbody>
									<tr>
										...
										<td><%=staname%></td>
										<td>
											<a href="order_del?oid=<%=oid%>" onclick="return del()"><span class="lnr lnr-trash"></span></a>
										</td>
										<%
									    if(staname.equals("配餐中")){
									    %>
									    <td>
											<a href="orders_state.jsp?oid=<%=oid%>"><span class="label label-success">完成</span></a>
											<!-- <a href="orders_state.jsp"><button type="button" class="btn btn-primary btn-sm" data-context="success">完成</button></a> -->
										</td>
									    <%
									    }
									    %>				
